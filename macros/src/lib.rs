...
use syn::{parse_macro_input, Data, DeriveInput, GenericParam, Generics, Ident, Lifetime, LifetimeParam, Member};
...
fn impl_derive_rustsbi_dynamic(name: &Ident, imp: DynamicImpl, generics: &Generics) -> TokenStream {
    ...
    let prober_generics = {
        ...
        let lifetime = Lifetime::new("'_lt", Span::call_site());
        ans.params.insert(0, GenericParam::Lifetime(LifetimeParam::new(lifetime)));
        ans
    };
